...
```
####  **Analysis:** We see that distribution of each variable. We dont see any non                             numerical numbers or any non catetorical labels that dont belong there
...
# Assumption 3: Assess Influentional Outliers that are numeric. Categorical predictors are reviewed in the histograms above
alias(model4)
...
```
#### **Analysis**: Assumption 1 met: We see that the outcome variable is binary.                              Assumption 2 met: Graphs of all continous predictor residuals with the                                       outcome variable show a straight line. A linear                                            relationship.                                                            Assumption 3 met: Cooks D is less than 0.5                                                 Assumption 4 met: VIF = 1.1 which less than 5.
Conclusion: All 4 assumptions met. So Logistic Regression is a valid model to test the                 dataset on.
...
Because the AUC and Youden's J are the same, with the predictability of the model at 86.5%, the same. I dont see any more we can do for this dataset, given its predictor variables are well known in other stroke studies. Logistic Regression appears to be the best fitting algorithm for prediciting stroke for this dataset. Using SMOTE might give an incremental increase in prediction, but given the well defined dataset and the common stroke risk factors, I dont think we need to run anymore tests. In summary, Logistic Regression is a very good algorithm to use on this dataset.
...
## **Slideshow Summary of Logistic Regression Study**

```{=html}
<div style="margin-top: 40px; text-align: center;">
  <button id="startSlideshowBtn" style="
    padding: 12px 30px;
    font-size: 16px;
    font-weight: bold;
...
">
  ...
  ">
    ...
    ">
      <h2 id="slideTitleDisplay" style="color: white; margin: 0; font-size: 24px;"></h2>
      <button id="closeSlideshowBtn" style="
    ...
    ">
      ...
      <div style="color: white; font-weight: bold; font-size: 16px;">
        Slide <span id="currentSlide">1</span> of <span id="totalSlides">0</span>
      </div>
...
<script>
  // Slideshow Data - Define your slides here
  const slides = [
    {
      ...
      content: `
        <h3>Key Topics</h3>
        <ul>
          <li>Background of Logistic Regression</li>
          <li>Research Logistic Regression</li>
          <li>To see how well Logistic Regression performes</li>
          <li>Verification of Logistic Regression in a rare event dataset</li>
        </ul>
      `
    },
    {
      ...
      content: `
        ...
        <ul>
          <li>Kaggle Dataset of 5110 observations by Fedesoriano with 12 variables</li>
          <li>Outcome variable: Stroke or No Stroke</li>
          <li>Predictor Variables: Gender, Age, Hypertension, Heart Disease, Marital Status, Work Type, Residence Type, Average Glucose Level, BMI, Smoking Status</li>
          <li>After cleaning (removing N/A's, unknowns): 3,357 cases with 11 predictors</li>
        </ul>
        <h3>Methods</h3>
        <ul>
          <li>Tested 4 assumptions of Logistic Regression</li>
          <li>Built 3 different models to test the dataset</li>
        </ul>
      `
    },
    {
      ...
      content: `
        <h3>Key Findings</h3>
        <ul>
          <li><strong>All assumptions were met:</strong> Binary outcome, linearity, no influential cases, no multicollinearity</li>
          <li><strong>Model Performance:</strong> All 3 models had the same AUC of 0.8285</li>
          <li><strong>Youden's J Point:</strong> Identical across all models</li>
          <li>The model fits the dataset well with 86.5% predictability</li>
          <li>No significant bias or separation issues detected</li>
        </ul>
        <h3>Interpretation</h3>
        <p>The consistency across all three models (Baseline, Firth, FLIC) indicates that the dataset is balanced enough to distinguish between stroke and non-stroke events without requiring advanced bias correction methods.</p>
      `
    },
    {
      title: "Conclusions",
      content: `
        ...
        <ul>
          <li>This is a valid dataset for studying logistic regression</li>
          <li>Dataset contains well-defined variables with no complex interactions</li>
          <li>Logistic Regression is an appropriate and effective algorithm for this stroke prediction task</li>
          <li>The clinical outcomes clearly demonstrate why logistic regression is common in healthcare</li>
          <li>Model achieves strong predictive accuracy (AUC = 0.8285)</li>
        </ul>
        <h3>Future Directions</h3>
        <ul>
          <li>Explore larger and more complex datasets to further validate findings</li>
          <li>Consider SMOTE for potential incremental improvements if needed</li>
          <li>Apply methodology to other healthcare datasets</li>
        </ul>
      `
    }
  ];
  ...
  function showSlide(index) {
    ...
    document.getElementById('slideTitleDisplay').textContent = slide.title;
    ...
    if (currentSlideIndex === 0) {
      document.getElementById('prevSlideBtn').style.opacity = '0.5';
      document.getElementById('prevSlideBtn').style.cursor = 'not-allowed';
    } else {
      document.getElementById('prevSlideBtn').style.opacity = '1';
      document.getElementById('prevSlideBtn').style.cursor = 'pointer';
    }

    if (currentSlideIndex === slides.length - 1) {
      document.getElementById('nextSlideBtn').style.opacity = '0.5';
      document.getElementById('nextSlideBtn').style.cursor = 'not-allowed';
    } else {
      document.getElementById('nextSlideBtn').style.opacity = '1';
      document.getElementById('nextSlideBtn').style.cursor = 'pointer';
    }
  }
  ...
  document.getElementById('closeSlideshowBtn').addEventListener('click', function() {
  ...
  document.addEventListener('keydown', function(event) {
    ...
    if (modal.style.display === 'block') {
      if (event.key === 'ArrowRight') {
        ...
        document.getElementById('closeSlideshowBtn').click();
      }
    }
  });
